...
student_username = input("Enter cadet username (not email) to disable:")

samaccountname = "samaccountname=" + student_username
# Get account object in AD
result = ad.search_s("dc=GYA, dc=local", ldap.SCOPE_SUBTREE, samaccountname) # TODO move base to .env
# console.print(result[0][1]["userAccountControl"][0])
if result[0][0] is not None:
    print("student was found in AD")
    userAccountControl = result[0][1]["userAccountControl"][0] # returns b'512' for active user
    ...
    # Setup modlist
    mod = modlist.modifyModlist({'userAccountControl': [b'512']},{'userAccountControl': [b'514']})
