...
from filemaker_api.filemaker_api import filemaker_get_records  # nopep8
...
def enrollmentsgen() -> None:
    ...
    for s in students:
        ...
        for section in sections:
            # Grab the first two characters from the section id (formatted like C50S1C5USHistory, or in the case of combined groups, C50S1DG3Government)
            sectionAndPeriod = re.search(
                "(?<=C\d\dS[1-2])([A-J]*)[1-9]", section["Section_id"])
            if sectionAndPeriod:
                section_group = sectionAndPeriod.group(1)

            # If Section_id is formatted as A1, B3, etc (not Contract1) AND If student group matches this section's group
            if (sectionAndPeriod) and (s["Group"] in section_group):
